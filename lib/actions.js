...
function Actions (Client) {
  ...
  /**
   * Change current client connection to specific node.
   * Reconnect only with this node.
   *
   * @param {number} attemptNumber
  ...
  Client.prototype.changeConnection = function changeConnection(attemptNumber, node, done) {
      ...
      // Check if the desired node is the current node
      if (node.id === client.nodeId) {
          debug('already connected to this client');
          done(null, client);
      } else {
