...
function sendRequest(connection, request, done) {
  ...
  function read() {
    var size = connection.read(4).readInt32BE(0);
    debug('Got response of size '+ size + 'b');

    done(null, connection.read(size));
    cleanUp();
  }
  function onErr(err) {
    debug(err);
...
function getWithConnection(connection, storeName, key, shouldRoute, done) {
  ...
  async.waterfall([
    function(next) {
      Client.sendRequest(connection, payload, next);
    },
    function(resp, next) {
      fs.writeFileSync(__dirname+'/../tmp.buff', resp);
      var test = Response.fromProto('get', resp);
      next(null, test);
    }
  ], done);
